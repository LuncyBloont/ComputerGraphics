...
bool simpleMesh(Edge* & elist, Vertex* & vlist, Face* & flist, int* eNum, int* vNum, int* fNum, GLfloat limit)
{
    ...
    GLfloat avgLen = 0.f;
    GLfloat avgBase = 0.f;
    std::map<GLfloat, std::pair<Edge*, GLfloat> > emap;
    for (int i = 0; i < *eNum; i++)
    {
        Vertex *f[] = { elist[i].end, elist[i].next->end };
        GLfloat subcost[] = { 0.f, 0.f };
        bool succ = true;
        GLfloat lerp = 0.0f;
        GLfloat cost = 0.0f;
        GLfloat base = 0.f;
        ...
        for (int j = 0; j < 2; j++)
        {
            ...
            do
            {
                ...
                GLfloat w = abs(dot(normalize(crs), dir));
                subcost[j] += w;
                cost += w;
                base += 1.f;
                p = p->invert->next;
